...
任何新 session 在 `<cwd>/` 收到第一条用户输入时（包括“聊聊”“报告状态”这类非任务输入），必须先完成 catch-up。未完成前，禁止直接闲聊，禁止回答“无 active goal / 后台任务”这类 runtime 状态。
...
然后再回答用户本轮意图。source of truth 是根 [`PROJECT.md`](<到根的相对路径>/PROJECT.md)。
````
