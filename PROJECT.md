...
注：本项目已从单 `Engineer` 升级为**持久 Agent team**。常驻 Worker 必须有独立 cwd；横切部署 / QA / 运维不建常驻 Worker，由 Orchestrator 按需派临时 SubAgent。
...
| 范围 | 启动方式 | 原因 |
|---|---|---|
| Infra / Deploy / Cloud | Orchestrator 从仓库根临时派发 | 横切整仓，无独立 cwd，不配常驻 Worker |
| QA / Migration / Release Gate | Orchestrator 从仓库根临时派发 | 横切整仓，无独立 cwd，不配常驻 Worker |
