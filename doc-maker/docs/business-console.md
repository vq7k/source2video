...
>
> **2026-05-18 更新**：新增轻量 L1 `/writing` 方向，目标是开箱即用、少配置、先出结果。具体设计原则见 [`lightweight-writing-workbench-design.md`](./lightweight-writing-workbench-design.md)。本文保留现有 legacy workbench 的业务契约和历史设计背景。
...
> 输入是不稳定的原始素材和条件规则；输出是多篇候选文本；系统用固定 Precheck + Eval 闭环把写法沉淀为可发布 Rule Package 的写法规则资产。
...
| Candidate Draft | 多条候选路径、独立 eval、选中文本反馈 | 单次 batch LLM call |
| Rule Package | 可复用写法规则资产：范围、来源、风险、eval 证据 | Codex/Claude 技术能力包 |
...
9. Rule Scope 必须支持“应用规则 / 不应用规则”的 A/B 对照，用来解释规则是否真的改善输出。
10. Rule Package 是产品层写法资产；Skill Package 是发布后的技术能力包，命名不能混用。
11. 参考来源必须展示覆盖率和过度仿写风险；高风险规则不能直接发布。
12. 历史 Job 反馈只能先沉淀为 dataset draft，进入正式 eval dataset 前必须人工确认。
13. v1 主入口 `/writing` 只保留最短闭环：生成、反馈再来一轮、定稿、规则包沉淀、观测。
...
| 输入 | 用途 | 示例 |
|---|---|---|
| 问题定义 | 本轮写作要解决什么沟通任务 | “给评审者解释为什么产品线从框架优先转向文本生产闭环” |
| 底稿 | 事实、观点、链接、转录稿、旧笔记 | 会议纪要、视频转录、素材摘录 |
| 交付 | 输出档案或文本形态 | 结构化讲解文档包、长文、决策复盘 |
| 写法参考 | 示例文章、账号风格、外部写作方法包 | 专业作家文章、公众号爆款结构 |
| 评审偏好 | 评审要求、禁忌、历史打分偏好 | “不要鸡汤，不要夸张标题” |

Baseline 设计原则：

> 系统可以预设流程、字段结构和治理闭环，但不能全局预设内容规则。字数、口播结构、TTS 标签、分镜格式、视觉风格等只能来自用户确认字段、已发布规则模板，或 LLM 在 Precheck 中给出的待确认建议。

默认 Output Profile：

| 档案 | 产物 |
|---|---|
| text artifact | 只声明当前节点生产文本产物，不声明字数、口播、TTS 或分镜规则 |

默认 Output Contract 为空：
...
| 场景 | 规则来源 | LLM 行为 |
|---|---|---|
| 无模板新任务 | Draft Rule Scope | Rule Scope LLM 自动提炼，结果标记为待确认 |
| 用户手动编辑 | 用户确认字段 | 不自动覆盖，只作为下一次 Precheck 输入 |
| 复用已发布模板 | Template Rule Scope | 不自动提炼规则，只加载模板 |

输入契约和生成前检查的关系：

- 输入契约只保存用户输入、用户确认字段和可追溯复制字段。
- Rule Scope / Precheck LLM 可以生成建议，但不能静默反写输入契约。
- 生成前检查是派生结果：`输入契约 + 已确认 Rule Scope -> Precheck LLM -> Content Brief / 依据边界 / 建议规则 / 风险`。
...
生成 Writing Rule Scope 成功后必须入库为 `Draft Job`：

| 阶段 | 入库 | 列表状态 |
|---|---|---|
| Quick Intake 空白 | 否 | 不显示 |
| 生成规则范围成功 | 是 | 草稿 / 规则范围已生成 |
| 确认规则范围并进入检查 | 是 | 待确认检查 |

原因：scope extraction 已经发生 LLM 调用、trace 和成本，刷新不能丢；Langfuse / local trace 必须绑定 `draftRunId`，避免变成无法关联的 adhoc trace。
...
### 2.2 Rule Scope A/B 对照与风险规则

Writing Rule Scope 确认页必须支持“应用规则 / 不应用规则”的 A/B 对照：同一 Quick Intake、Reference Paste、Output Contract 下，对比 baseline 输出和 applied-scope 输出，让用户看到规则带来的结构、语气、禁忌和检查点变化。这个对照是规则理解与 eval 输入，不是新的最终候选批次。

| 产品规则 | 要求 |
|---|---|
| A/B 对照 | 至少展示 applied rules 与 no rules 两侧差异，支持用户删除有问题的 scope item 后重算或重预览 |
| 来源覆盖率 | 每条 Rule Scope / Writing Rules Candidate 必须能说明覆盖了哪些参考片段、source note 或人工反馈；未覆盖的关键来源进入 Risk List |
| 过度仿写风险 | 风险项必须区分“借结构”和“复制表达”；高相似表达、连续措辞复用、账号风格污染进入 blocked 或 warning |
| 发布边界 | 有来源缺口或过度仿写高风险的规则只能停留在 candidate / Rule Package draft，不能进入 Published Skill Package |
...
历史 Job 反馈可以沉淀为 dataset draft，但不能自动进入正式 eval dataset。dataset draft 至少包含：Job Spec、Rule Snapshot、候选文本、选中文本片段、反馈标签、系统归因、最终选择结果和风险状态。用户确认后才可进入后续 Langfuse / Braintrust dataset loop。
...
## 5. Rule Package 与 Skill Package 生命周期

Writing Rules Candidate 是本轮规则候选，不是 prompt，也不是标准 Skill。Rule Package 是产品层写法资产包，用来沉淀规则范围、来源覆盖、风险状态、eval 证据和版本说明。Skill Package 才是发布后的技术能力包，可安装、可测试、可导出。

命名边界：

| 名称 | 含义 | 不能做什么 |
|---|---|---|
| Writing Rule Scope | 本轮从 Quick Intake / Reference Paste 提炼的临时范围 | 不能叫 Skill，不能跨 Job 自动复用 |
| Rule Package | 多轮 Job 后沉淀的写法规则资产，可继续 eval 和修订 | 不能直接等同 `SKILL.md` |
| Published Skill Package | 已发布的技术能力包，包含 `SKILL.md + resources + scripts + tests` | 不能被单次人工反馈直接改写 |
...
当前 baseline 原型支持从定稿候选生成并发布 Rule Package；暂不支持 UI 发布 Published Skill Package。
...
| 人工反馈直接改 Published Skill Package | 评审偏好不稳定，必须先进入 candidate |
...
- [x] 实现 Rule Scope “应用规则 / 不应用规则” A/B 对照预览。
- [x] 引入 Rule Package 命名边界，避免把候选规则或提示词叫 Skill Package。
- [x] 在 Scope / Precheck 中展示来源覆盖率和过度仿写风险。
- [ ] 将历史 Job 反馈沉淀为 dataset draft，人工确认后再进入正式 eval dataset。
- [ ] 暂不做 Source Store / RAG / workflow builder；当前只保留 Reference Paste 与既有 trace/eval 闭环。
