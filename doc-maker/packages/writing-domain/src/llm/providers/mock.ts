  ...
  return {
    ...
    checks: [
      ...
      },
      {
        ...
        guidance: "低置信规则只能作为候选，不应发布为 Rule Package。",
      },
    ],
  };
}
...
function buildPrecheckDraft(input: GeneratePrecheckRequest): PrecheckDraft {
  ...
  const artifactType = outputContract.artifactType || "未指定（由 Precheck LLM 建议）";
  const lengthRange = outputContract.lengthRange || "未指定（由 Precheck LLM 建议）";
  const structure = outputContract.structure || "未指定（由 Precheck LLM 建议）";
  const formatRules = outputContract.formatRules || "未指定（由 Precheck LLM 建议）";
  const groundingRules = outputContract.groundingRules || "使用输入材料和已确认 Rule Scope，不补写未知事实。";
  ...
  return {
    ...
    groundingBrief: `底稿边界：${jobSpec.source || "未提供底稿"}。交付契约：${artifactType} / ${lengthRange} / ${structure}。Handoff：${outputContract.downstreamHandoff || outputProfile.name}。`,
    writingRulesCandidate: [
      `产物：${artifactType}；长度：${lengthRange}；格式：${formatRules}`,
      `依据：${groundingRules}`,
...
function buildCandidateDrafts(input: GenerateCandidatesRequest): CandidateDraft[] {
  ...
  const variantInstruction = input.variantInstruction ?? "默认候选路径";
  const titleByVariant = [
    "先给结论",
    "按证据展开",
    "压缩成口播",
  ][variantIndex] ?? `独立路径 ${variantIndex + 1}`;

  return Array.from({ length: input.candidateCount }, (_, index) => ({
    title: `${input.jobSpec.title || "本轮主题"}：${titleByVariant}`,
    summary:
      variantIndex === 0
    ...
    excerpt: [
      ...
      `本候选是第 ${variantIndex + 1}/${input.variantCount ?? input.candidateCount} 条独立生成路径，角度：${variantInstruction}。`,
      ...
      `用户补充的 Topic Boundary 硬事实：${contextLine}。这些事实优先于规则草稿，不能被改写成普通风格偏好。`,
