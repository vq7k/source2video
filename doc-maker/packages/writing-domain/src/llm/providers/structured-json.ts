...
import type {
  ...
  RuleScopeDraft,
  RuleScopeDraftItem,
} from "@doc-maker/writing-domain/llm/types";
...
function normalizeCandidateDraft(value: unknown, index: number): CandidateDraft | null {
  ...
  return {
    ...
    excerpt: text,
    rationale: boundedStringValue(record.rationale, "模型未返回生成理由。", 600),
    ...
    },
    {
      ...
      content: JSON.stringify({
        ...
        constraints: [
          ...
          "If outputContract fields are empty, infer candidate suggestions from jobSpec and ruleScope and mark them as suggestions inside writingRulesCandidate; do not treat empty contract fields as system defaults.",
        ],
        input,
      }),
    },
  ...
  return [
    {
      role: "system" as const,
      content:
        "你是 doc-maker 的候选生成节点。只输出 JSON，不输出 markdown fence。生成的是可评审正文候选，不生成视觉指导、TTS 改写或下游视频脚本。必须严格遵守输出契约、生成前检查候选和规则快照。userProvidedContext 是用户补充的 Topic Boundary 硬事实，优先级高于 ruleSnapshot、rulePatches 和 recentFeedback，必须原样保留其事实含义，不能概括成风格偏好。",
    },
    {
      role: "user" as const,
      content: JSON.stringify({
        ...
        constraints: [
          ...
          input.candidateCount > 1
            ...
            : "Generate only this one candidate path; do not mention alternative versions.",
          input.variantInstruction
            ? `This independent candidate path must follow this angle: ${input.variantInstruction}`
            : "Use a concrete angle that can be evaluated independently.",
          "Do not score the candidate. Evaluation is handled by a separate candidate_eval node.",
          "不要编造输入契约 / 依据边界之外的事实。",
          ...
          "Priority order: userProvidedContext > jobSpec/source > precheckRun/groundingBrief > ruleSnapshot > activeRulePatches > recentFeedback.",
          "If userProvidedContext is non-empty, candidates must respect those facts explicitly; do not weaken them into generic style advice.",
        ],
        input: {
          round: input.round,
          ...
          userProvidedContext: input.userProvidedContext.filter((item) => item.status === "active").slice(-8),
