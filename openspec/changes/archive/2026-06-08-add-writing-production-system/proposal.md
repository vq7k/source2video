...
- 新增前置 `Precheck` 环节：把混乱原始输入清洗为固定结构的 `Content Brief`、`Writing Rules Candidate` 和本轮 `Eval Profile`。
- 新增下一步输入初始化边界：先做 `Manual Rule Scope`，通过 Quick Intake + 可选 Reference Paste 提炼本轮 Writing Rule Scope；暂不做 Source Store / RAG。
...
- 新增 Rule-to-Skill Package 生命周期：从本期候选规则资产开始，经多轮 eval 与人工反馈稳定后，才允许进入发布设计；当前 baseline 不发布。
...
- `precheck-normalization`: 定义 Precheck 如何把混乱输入清洗为 Content Brief、Writing Rules Candidate、Eval Profile 与风险清单。
- `generation-eval-loop`: 定义多篇候选生成、自动 eval、轻反馈、规则草稿和下一轮迭代闭环。
- `writing-skill-lifecycle`: 定义 Writing Rules Candidate 如何迭代，并在稳定后发布为 Skill Package。
...
- 受影响产品语言：Episode、source、artifact package 需要映射到 Writing Job、Content Brief、Skill Package、Eval Profile、Candidate Draft、Output Profile。
